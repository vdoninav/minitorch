...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    if a < 10:
        epsilon = 0.01
        assert (
            sigmoid(a + epsilon) > sigmoid(a)
            or abs(sigmoid(a + epsilon) - sigmoid(a)) < 1e-6
        )
...
def test_symmetric() -> None:
    ...
    @given(small_floats, small_floats)
    def _test_mul_symmetric(a: float, b: float) -> None:
...
def test_distribute() -> None:
    ...
    @given(small_floats, small_floats, small_floats)
    def _test_distributive(x: float, y: float, z: float) -> None:
        left = mul(z, add(x, y))
...
def test_other() -> None:
    ...
    @given(small_floats)
    def _test_additive_identity(a: float) -> None:
        assert_close(add(a, 0.0), a)
        assert_close(add(0.0, a), a)

    @given(small_floats)
    def _test_multiplicative_identity(a: float) -> None:
        assert_close(mul(a, 1.0), a)
        assert_close(mul(1.0, a), a)

    @given(small_floats)
    def _test_double_negation(a: float) -> None:
        assert_close(neg(neg(a)), a)

    _test_additive_identity()
    _test_multiplicative_identity()
    _test_double_negation()
